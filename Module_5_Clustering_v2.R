...
apRes <- apcluster(negDistMat(r=2), cho.data)
apRes
...
matplot(t(cho.data[unlist(apRes[1]),]),type="l",xlab="time",ylab="log expression value")
matplot(t(cho.data[unlist(apRes[2]),]),type="l",xlab="time",ylab="log expression value")
matplot(t(cho.data[unlist(apRes[3]),]),type="l",xlab="time",ylab="log expression value")
matplot(t(cho.data[unlist(apRes[4]),]),type="l",xlab="time",ylab="log expression value")
matplot(t(cho.data[unlist(apRes[5]),]),type="l",xlab="time",ylab="log expression value")
par(oPar)


# try the same with the full 237 row dataset
# maximizes the clustering based on mathematical algorithms - but they're not going to substitute good biology. 
...
# the result here is plotting a density function dependent on the clustering of the data. The RED line represents the mean. 
...
# do schoastic clustering, then affinity propogation, then you'll be able to pick up genes that are of interest in the future. 
# this helps oyu find out what is statistically signfiicant etc. 
